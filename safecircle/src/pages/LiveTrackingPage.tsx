...
import type { AlertMessage, JourneyProgressResponse, LiveLocationMessage } from '@/types';
...
export default function LiveTrackingPage() {
  ...
  // Receive automatic SOS events instantly. The UI changes through WebSocket
  // instead of waiting for the 8-second progress poll or a page reload.
  useEffect(() => {
    if (!journeyId) return;

    const unsubscribe = socketService.subscribe<AlertMessage>(
      wsTopics.alert(journeyId),
      (message) => {
        if (message.status !== 'ACTIVE') return;

        setProgress((current) =>
          current
            ? { ...current, sosTriggered: true, currentRisk: current.currentRisk }
            : current
        );

        setLiveCoords([message.latitude, message.longitude]);
        toast.error('Automatic SOS triggered — your trusted contacts are being alerted.');
      }
    );
